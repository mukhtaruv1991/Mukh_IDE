...
def get_main_menu():
    ...
    return InlineKeyboardMarkup(k), "🤖 *Mukh IDE (v20.0)*\n\nأهلاً بك! اختر إجراءً للبدء."
def get_project_dashboard_menu(project_name):
    k = [[InlineKeyboardButton("🚀 نشر", callback_data=f'publish_{project_name}')], [InlineKeyboardButton("🛡️ نسخ احتياطي", callback_data=f'backup_{project_name}')], [InlineKeyboardButton("🔙 العودة", callback_data='list_projects')]]
    return InlineKeyboardMarkup(k), f"🛠️ *مشروع: {project_name}*\n\nاختر إجراءً."
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if command == 'main_menu':
        ...
        if not projects:
            k = [[InlineKeyboardButton("➕ إضافة مشروع", callback_data='add_project_start')]]
            await query.edit_message_text("لا توجد مشاريع. هل تريد إضافة مشروع؟", reply_markup=InlineKeyboardMarkup(k)); return
        k = [[InlineKeyboardButton(p, callback_data=f'project_{p}')] for p in projects]
        k.append([InlineKeyboardButton("🔙 القائمة الرئيسية", callback_data='main_menu')])
        await query.edit_message_text("📂 *المشاريع الحالية*:", reply_markup=InlineKeyboardMarkup(k), parse_mode=ParseMode.MARKDOWN)
    elif command.startswith('project_'):
        project_name = command.split('_', 1)[1]; context.user_data['current_project'] = project_name
        reply_markup, text = get_project_dashboard_menu(project_name)
        await query.edit_message_text(text, reply_markup=reply_markup, parse_mode=ParseMode.MARKDOWN)
    elif command == 'add_project_start':
        k = [[InlineKeyboardButton("📁 من Termux", callback_data='import_termux')], [InlineKeyboardButton("🔙 رجوع", callback_data='main_menu')]]
        await query.edit_message_text("➕ *إضافة مشروع جديد*\n\nمن أين تريد استيراد المشروع؟", reply_markup=InlineKeyboardMarkup(k), parse_mode=ParseMode.MARKDOWN)
    elif command == 'import_termux':
        await query.edit_message_text("أرسل المسار الكامل للمشروع (مثال: `~/my-app`).", parse_mode=ParseMode.MARKDOWN)
        context.user_data['state'] = 'awaiting_import_path'
    elif command.startswith('publish_'):
        project_name = command.split('_', 1)[1]
        await query.edit_message_text(f"🚀 جاري نشر: `{project_name}`...", parse_mode=ParseMode.MARKDOWN)
        ...
        project_name = command.split('_', 1)[1]
        await query.edit_message_text(f"🛡️ جاري النسخ الاحتياطي: `{project_name}`...", parse_mode=ParseMode.MARKDOWN)
        output = run_core_command(f"backup '{project_name}'")
        backup_file_path = output.split(":")[-1].strip()
        await context.bot.send_document(chat_id=query.effective_chat.id, document=open(backup_file_path, 'rb'), caption=f"نسخة احتياطية: {project_name}")
        reply_markup, _ = get_project_dashboard_menu(project_name)
        await query.edit_message_text("✅ تم إنشاء النسخة الاحتياطية.", reply_markup=reply_markup)
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    state = context.user_data.get('state'); text = update.message.text
    if state == 'awaiting_import_path':
        await update.message.reply_text(f"⏳ جاري استيراد: `{text}`...", parse_mode=ParseMode.MARKDOWN)
        ...
        if "ERROR" in project_name: await update.message.reply_text(f"❌ فشل:\n`{project_name}`", parse_mode=ParseMode.MARKDOWN)
            ...
            await update.message.reply_text(f"✅ تم الاستيراد: `{project_name}`.\n\nأرسل رابط مستودع GitHub لربطه.", parse_mode=ParseMode.MARKDOWN)
            context.user_data['state'] = 'awaiting_repo_url'; context.user_data['import_project_name'] = project_name
    elif state == 'awaiting_repo_url':
        project_name = context.user_data.get('import_project_name')
        await update.message.reply_text(f"⏳ جاري ربط `{project_name}`...", parse_mode=ParseMode.MARKDOWN)
        output = run_core_command(f"init '{project_name}' '{text}'")
        await update.message.reply_text(f"✅ *نتيجة الربط:*\n```\n{output}\n```\n\nتم تجهيز المشروع!", parse_mode=ParseMode.MARKDOWN)
        context.user_data.clear()
        reply_markup, text = get_project_dashboard_menu(project_name)
        await update.message.reply_text(text, reply_markup=reply_markup, parse_mode=ParseMode.MARKDOWN)
def bot_main():
    if not TOKEN: print("FATAL: Bot token not found."); return
    app = Application.builder().token(TOKEN).build()
    app.add_handler(CommandHandler("start", start)); app.add_handler(CallbackQueryHandler(button_handler))
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, text_handler))
    print("🤖 Mukh IDE Bot (v20.0) is running..."); app.run_polling()
...
EOR
        ...
        git add .
        git commit -m "Deploy/Update Mukh IDE Tools (v4.2)"
